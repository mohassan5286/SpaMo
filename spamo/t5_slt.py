...
class FlanT5SLT(AbstractSLT):
    ...
    Note: Now uses Qwen2-0.5B-Instruct as the underlying language model.
    """
    def __init__(
        self, 
        ...
        **kwargs
        ...
        self.prompt = prompt
        self.model_name = model_name if model_name else "Qwen/Qwen2-0.5B-Instruct"
    ...
    def _apply_lora(self) -> None:
        """Apply LoRA adapter to the Qwen2 model."""
        ...
        print("LoRA adapter applied to Qwen2 model.")

    def _freeze_model(self) -> None:
        """Freeze the Qwen2 model parameters."""
        self.t5_model.eval()
        for params in self.t5_model.parameters():
            params.requires_grad = False
        print("Qwen2 model frozen.")
    ...
    def prepare_models(self, t5_model: str) -> None:
        ...
        Args:
            t5_model: Name or path of the model to use (Qwen2)
        """
        
        # Load the Qwen2 model (causal LM)
        ...
        if self.t5_tokenizer.pad_token is None:
            ...
            self.t5_model.config.pad_token_id = self.t5_tokenizer.eos_token_id
    ...
    def prepare_inputs(
        ...
        """
        Prepare combined inputs for the Qwen2 model.
        ...
        # For Qwen2 causal LM, we need to format prompts differently
        # Add instruction format if needed
        prompts = [f"{p}\nTranslation:" for p in prompts]
        ...
        input_embeds = self.t5_model.model.embed_tokens(input_tokens.input_ids)
        ...
        # For causal LM, prepare labels differently
        # Shift targets: labels are input_ids shifted, with -100 for padding
        targets = output_tokens.input_ids.clone()
    ...
    def get_inputs(self, batch: List) -> Dict:
        ...
        # Only call derangement if the list has enough items
    ...
    def visual_textual_align(self, visual_outputs: torch.Tensor, visual_masks: torch.Tensor, samples: Dict) -> torch.Tensor:
        ...
        # Get text embeddings
        text_embeds = self.t5_model.model.embed_tokens(output_tokens.input_ids)
    ...
    def shared_step(self, inputs: Dict, split: str, batch_idx: int) -> Tuple[torch.Tensor, Dict]:
        ...
        if self.cross_modal_align:
            ...
            if self.warm_up_steps is None and not self.combined_loss:
                ...
                # Prepare full input embeddings (visual + prompt + target)
                target_embeds = self.t5_model.model.embed_tokens(output_tokens.input_ids)
                full_embeds = torch.cat([input_embeds, target_embeds], dim=1)
                full_mask = torch.cat([input_masks, output_tokens.attention_mask], dim=1)
                
                # Prepare labels: -100 for prompt positions, actual tokens for target
                labels = torch.full((input_embeds.shape[0], input_embeds.shape[1]), -100, dtype=torch.long, device=self.device)
                labels = torch.cat([labels, targets], dim=1)
                
                # Forward pass through Qwen2 model
        ...
        if split != "train":
            ...
            # For causal LM, we need to extract only the generated part
            ...
            reference_strings = [ref.lower().strip() for ref in reference_strings]
